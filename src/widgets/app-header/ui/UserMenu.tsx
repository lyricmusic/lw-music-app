...
export function UserMenu({ onClose, open }: UserMenuProps) {
  ...
  return (
    <>
      {open && (
        <div
          className="absolute -right-1 -top-1 z-10 w-[min(280px,calc(100vw-16px))] overflow-hidden rounded-2xl border border-[#4A2B6D] bg-[#24143D] text-[#F8F3FF] shadow-2xl"
          id="user-menu"
          role="menu"
        >
          ...
          <List>
            {user?.isAnonymous && (
              <ListItem disablePadding sx={{ borderTop: '1px solid #4A2B6D' }}>
                <ListItemButton onClick={openSaveAccountDialog} role="menuitem">
                  <MemberIcon
            ...
            <ListItem disablePadding sx={{ borderTop: '1px solid #4A2B6D' }}>
              <ListItemButton onClick={openCharacterEditor} role="menuitem">
                <MemberIcon
                  className="fill-light-brand mr-2"
                  sx={{ width: '16px' }}
                />
            ...
            <ListItem disablePadding sx={{ borderTop: '1px solid #4A2B6D' }}>
              <ListItemButton onClick={handleLogout} role="menuitem">
                <LogoutIcon
                  className="fill-light-brand mr-2"
                  sx={{ width: '16px' }}
                />

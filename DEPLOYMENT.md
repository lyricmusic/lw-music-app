...
Если нужно применить изменённый `deploy/nginx/syncly.lyricweb.ru.conf`, запустить
`Build develop` вручную с параметром `apply_nginx_config=true`. Обычный push не
меняет активную Nginx-конфигурацию. Ручной режим создаёт резервную копию,
проверяет `nginx -t`, повторно устанавливает уже действующий TLS-сертификат без
досрочного обновления и автоматически возвращает предыдущий конфиг при ошибке.
